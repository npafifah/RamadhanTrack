...
const overallProgress = computed(() => {
  ...
  return Math.round((p(tilawah.value, 30) + p(sholat.value, 30) + p(sedekah.value, 30) + p(qiyamul.value, 30)) / 4)
})
  ...
  <div :class="['min-h-screen pb-24 transition-all duration-500', isDark ? 'bg-slate-950 text-slate-100' : 'bg-stone-50 text-slate-900']">
    ...
    <div v-if="showDuaModal" class="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
      <div :class="['w-full max-w-2xl rounded-[2.5rem] p-8 max-h-[85vh] overflow-y-auto relative', isDark ? 'bg-slate-900' : 'bg-white shadow-2xl']">
        ...
        <h2 class="text-2xl font-black italic text-emerald-500 mb-6">Dua Library</h2>
    ...
    <main id="app-capture" class="max-w-4xl mx-auto px-6 space-y-6">
      ...
      <div v-if="activeTab === 'dashboard'" class="space-y-6 animate-in">
        <div :class="['p-8 rounded-[2.5rem] border overflow-hidden relative', isDark ? 'bg-slate-900 border-slate-800' : 'bg-white shadow-xl']">
      ...
      <div v-if="activeTab === 'ibadah'" class="space-y-6 animate-in">
        <div class="grid grid-cols-2 gap-4">
          <div v-for="(val, key) in { tilawah: 'Quran', sholat: 'Sholat', sedekah: 'Sedekah', qiyamul: 'Tarawih' }" :key="key"
            :class="['p-6 rounded-[2.5rem] border', isDark ? 'bg-slate-900 border-slate-800' : 'bg-white shadow-sm']">
            <p class="text-[10px] font-black opacity-40 uppercase mb-4">{{ val }}</p>
            <div class="flex items-center justify-between">
              <span class="text-4xl font-black italic tracking-tighter">{{ (key === 'tilawah' ? tilawah : key === 'sholat' ? sholat : key === 'sedekah' ? sedekah : qiyamul) }}</span>
              <button @click="key === 'tilawah' ? tilawah++ : key === 'sholat' ? sholat++ : key === 'sedekah' ? sedekah++ : qiyamul++" 
                class="w-12 h-12 bg-emerald-500 rounded-2xl text-white font-bold">+</button>
            </div>
          </div>
        </div>
        ...
        <div :class="['p-8 rounded-[2.5rem] border', isDark ? 'bg-slate-900 border-slate-800' : 'bg-white']">
      ...
      <div v-if="activeTab === 'spirituil'" class="space-y-6 animate-in">
        <div :class="['p-8 rounded-[2.5rem] border', isDark ? 'bg-slate-900 border-slate-800' : 'bg-white']">
          ...
          <input v-model="ramadanGoals" placeholder="Tulis target besarmu..." class="bg-transparent w-full text-xl font-medium outline-none">
        </div>
  ...
  <div :class="['p-6 rounded-[2rem] border', isDark ? 'bg-slate-900 border-slate-800' : 'bg-white shadow-sm']">
    ...
    <textarea v-model="iftarMenu" rows="2" class="bg-transparent w-full text-xs outline-none resize-none" placeholder="Isi menu buka..."></textarea>
        ...
        <div :class="['p-8 rounded-[2.5rem] border', isDark ? 'bg-slate-900 border-slate-800' : 'bg-white shadow-xl']">
          ...
          <textarea v-model="reflection" class="w-full bg-transparent resize-none outline-none text-sm leading-relaxed h-40" placeholder="Apa hikmah hari ini?"></textarea>
        </div>
        ...
        <button @click="showDuaModal = true" class="w-full p-8 rounded-[3rem] bg-gradient-to-br from-emerald-500 to-teal-600 text-white text-center shadow-xl">
          <h3 class="text-2xl font-black italic mb-1">📖 BUKA DOA</h3>
        </button>
      </div>
